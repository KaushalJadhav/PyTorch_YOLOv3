...
class YOLOLayer(nn.Module):
    ...
    def forward_losses(self,fsize,labels,output,pred):
        # target assignment
        tgt_mask = torch.zeros(self.batchsize, self.n_anchors,fsize, fsize,4 + self.n_classes).type(self.dtype)
        obj_mask = torch.ones(self.batchsize, self.n_anchors,fsize, fsize).type(self.dtype)
        tgt_scale = torch.zeros(self.batchsize, self.n_anchors,fsize, fsize, 2).type(self.dtype)
        target = torch.zeros(self.batchsize, self.n_anchors,fsize, fsize, self.n_ch).type(self.dtype)
